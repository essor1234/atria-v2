...
from atria.core.context_engineering.memory import AgentResponse
...
class IterationMixin:
    ...
    def _handle_llm_error(self, response: dict, ctx) -> "LoopAction":
        ...
        if "interrupted" in error_text.lower():
            ...
            self._last_error = error_text
            run_sync(self.session_manager.add_message(fallback, self.config.auto_save_interval))
    ...
    def _handle_no_tool_calls(self, ctx, content: str, raw_content: Optional[str]) -> "LoopAction":
        ...
        # but only when tools were actually used this session (real work to verify).
        # Skip the nudge for pure conversational turns to avoid double responses.
        tools_were_used = any(msg.get("role") == "tool" for msg in ctx.messages)
