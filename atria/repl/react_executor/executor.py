...
from atria.repl.react_executor.thinking import ThinkingMixin
from atria.repl.react_executor.tool_processing import ToolProcessingMixin
from atria.repl.react_executor.session_persistence import SessionPersistenceMixin
from atria.repl.react_executor.iteration import IterationMixin
...
if TYPE_CHECKING:
    ...
    from atria.models.config import Config
    from atria.repl.llm_caller import LLMCaller
    from atria.repl.tool_executor import ToolExecutor
    from atria.core.runtime.approval import ApprovalManager
    from atria.core.context_engineering.history import UndoManager
    from atria.core.debug.session_debug_logger import SessionDebugLogger
...
class ReactExecutor(ThinkingMixin, ToolProcessingMixin, SessionPersistenceMixin, IterationMixin):
    ...
    def _drain_injected_messages(self, ctx: IterationContext, max_per_drain: int = 3) -> int:
        ...
        while count < max_per_drain:
            ...
            run_sync(self.session_manager.add_message(user_msg, self.config.auto_save_interval))
    ...
    def execute(
        ...
        from atria.core.context_engineering.validated_message_list import ValidatedMessageList
        ...
        if self._snapshot_manager is None:
            try:
                from atria.core.context_engineering.history.snapshot import SnapshotManager
    ...
    def _maybe_compact(self, ctx: IterationContext) -> None:
        ...
        from atria.core.context_engineering.compaction import OptimizationLevel
