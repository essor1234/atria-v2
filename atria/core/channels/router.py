...
from atria.core.channels.reset_policies import should_reset_session
from atria.core.channels.workspace_selector import WorkspaceSelector
from atria.core.context_engineering.history.session_manager import SessionManager
from atria.models.message import ChatMessage, InputProvenance, Role
...
class MessageRouter:
    ...
    async def _resolve_session(self, message: InboundMessage) -> Session:
        ...
        metadata = await self._session_manager.find_session_by_channel_user(
            channel=message.channel,
            user_id=message.user_id,
            thread_id=message.thread_id,
        )
        ...
        if metadata:
            ...
            session = await self._session_manager.load_session(metadata.id)
