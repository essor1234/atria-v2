...
class MemoryTools:
    ...
    - Project-level: .atria/memory/*.md (and ATRIA.md)
    - User-level: ~/.atria/memory/*.md
    """
    ...
    def _get_memory_dirs(self) -> list[Path]:
        ...
        dirs = []
        # Project-level
        project_memory = self._working_dir / ".atria" / "memory"
    ...
    def _get_memory_files(self) -> list[Path]:
        ...
        files = []
        # Check for ATRIA.md at project root
        atria_md = self._working_dir / "ATRIA.md"
        if atria_md.exists():
            files.append(atria_md)
        # Check for .atria/MEMORY.md
        project_memory_md = self._working_dir / ".atria" / "MEMORY.md"
    ...
    def search(
        ...
        if not files:
            return {
                "success": True,
                "output": "No memory files found. Memory files are stored in .atria/memory/ (project) or ~/.atria/memory/ (user).",
                "matches": [],
            }
