...
class ConfigManager:
    ...
    def load_config(self) -> AppConfig:
        ...
        1. Local project config (.atria/settings.json)
        2. Global user config (~/.atria/settings.json)
        3. Default values
        """
        # Start with defaults
        config_data: dict = {}
        ...
        for env_source in (global_data, local_data):
            for key, value in env_source.get("env", {}).items():
    ...
    def ensure_directories(self) -> None:
        ...
        # Expand paths
        atria_dir = Path(config.atria_dir).expanduser()
        log_dir = Path(config.log_dir).expanduser()

        # Create directories
        atria_dir.mkdir(parents=True, exist_ok=True)
    ...
    def load_context_files(self) -> list[str]:
        """Load ATRIA.md context files hierarchically.
        ...
        project_context = self.working_dir / "ATRIA.md"
        ...
        while current != current.parent:
            subdir_context = current / "ATRIA.md"
    ...
    def user_skills_dir(self) -> Path:
        """Get user-global skills directory (~/.atria/skills/)."""
        return get_paths(self.working_dir).global_skills_dir

    @property
    def project_skills_dir(self) -> Path | None:
        """Get project-local skills directory (<project>/.atria/skills/).
    ...
    def _get_bundle_skill_dirs(self) -> list[Path]:
        ...
        bundle_dirs = []
        try:
            from atria.core.plugins import PluginManager
    ...
    def load_custom_agents(self) -> list[dict[str, Any]]:
        ...
        1. ~/.atria/agents.json (user global JSON)
        2. ~/.atria/agents/*.md (user global markdown)
        3. <project>/.atria/agents.json (project local JSON)
        4. <project>/.atria/agents/*.md (project local markdown)
