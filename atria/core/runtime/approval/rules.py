...
Persistent rules are stored in:
  - User-global: ~/.atria/permissions.json
  - Project-scoped: .atria/permissions.json
"""
...
class ApprovalRulesManager:
    ...
    def add_persistent_rule(self, rule: ApprovalRule, *, scope: str = "user") -> None:
        ...
        Args:
            ...
            scope: "user" for ~/.atria/permissions.json,
                   "project" for .atria/permissions.json in the project dir.
        """
        self.add_rule(rule)
    ...
    def clear_persistent_rules(self, *, scope: str = "user") -> int:
        ...
        if scope in ("project", "all") and self._project_dir:
            project_path = Path(self._project_dir) / ".atria" / "permissions.json"
    ...
    def _save_persistent_rules(self, *, scope: str = "user") -> None:
        ...
        if scope == "user":
            ...
            path = Path(self._project_dir) / ".atria" / "permissions.json"
        else:
            return
