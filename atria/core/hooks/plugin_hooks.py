...
Plugins are Python modules loaded from ~/.atria/plugins/ and .atria/plugins/
...
Example plugin (~/.atria/plugins/my_plugin.py):

    from atria.core.hooks.plugin_hooks import PluginHook
...
class PluginHookManager:
    ...
    1. ~/.atria/plugins/*.py (user-level)
    2. .atria/plugins/*.py (project-level)
    ...
    def _load_plugin(self, plugin_path: Path) -> int:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"atria_plugin_{plugin_path.stem}", plugin_path
            )
