...
class WebUICallback(BaseUICallback):
    ...
    # ------------------------------------------------------------------
    # Deep research taxonomy review
    # ------------------------------------------------------------------

    def request_taxonomy_review(
        self,
        job_id: str,
        review_request_id: str,
        event_payload: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """Register pending review, broadcast `event_payload`, then block.

        Registration MUST happen before the WS broadcast so the websocket
        handler can resolve the review even if the user responds instantly.
        Returns:
            Dict with keys: action ("accept"|"regenerate"), topic, taxonomy, depth.
        """
        done_event = threading.Event()

        self.state.add_pending_taxonomy_review(
            review_request_id,
        ...
        if event_payload is not None:
            msg_type = event_payload.get("type", "deep_research_taxonomy_ready")
            data = {k: v for k, v in event_payload.items() if k != "type"}
            data["session_id"] = self.session_id
            self._broadcast({"type": msg_type, "data": data})

        logger.info(f"Blocking for taxonomy review: {review_request_id} / job {job_id}")

        wait_timeout = 600  # 10 minutes
        if not done_event.wait(timeout=wait_timeout):
            logger.warning(f"Taxonomy review {review_request_id} timed out")
            self.state.clear_taxonomy_review(review_request_id)
            return {"action": "accept"}

        pending = self.state.get_pending_taxonomy_review(review_request_id)
        self.state.clear_taxonomy_review(review_request_id)
        if not pending:
            return {"action": "accept"}

        action = pending.get("action", "accept")
        logger.info(f"Taxonomy review {review_request_id} resolved: action={action}")
        return {
            "action": action,
            "topic": pending.get("topic"),
            "taxonomy": pending.get("taxonomy"),
            "depth": pending.get("depth", "standard"),
    ...
    # ------------------------------------------------------------------
    # Image push (used by send_image tool)
    # ------------------------------------------------------------------

    def on_image(self, src: str, mime_type: str = "", caption: str = "") -> None:
        """Broadcast an image message to the chat UI."""
        self._broadcast(
            {
