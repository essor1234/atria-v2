...
class TestPathsGlobal:
    ...
    def test_global_dir_default(self, tmp_path, monkeypatch):
        """Test global dir defaults to ~/.atria/."""
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.delenv(ENV_ATRIA_DIR, raising=False)
        reset_paths()
        paths = Paths()
        assert paths.global_dir == tmp_path / ".atria"

    def test_global_dir_env_override(self, tmp_path, monkeypatch):
        """Test ATRIA_DIR environment variable overrides default."""
        custom_dir = tmp_path / "custom-swecli"
        monkeypatch.setenv(ENV_ATRIA_DIR, str(custom_dir))
        reset_paths()
        paths = Paths()
    ...
    def test_global_settings(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_settings == tmp_path / ".atria" / "settings.json"

    def test_global_sessions_dir_default(self, tmp_path, monkeypatch):
        """Test global sessions directory defaults to ~/.atria/sessions/."""
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.delenv(ENV_ATRIA_DIR, raising=False)
        monkeypatch.delenv(ENV_ATRIA_SESSION_DIR, raising=False)
        reset_paths()
        paths = Paths()
        assert paths.global_sessions_dir == tmp_path / ".atria" / "sessions"

    def test_global_sessions_dir_env_override(self, tmp_path, monkeypatch):
        """Test ATRIA_SESSION_DIR environment variable overrides default."""
        custom_sessions = tmp_path / "custom-sessions"
        monkeypatch.setenv(ENV_ATRIA_SESSION_DIR, str(custom_sessions))
        reset_paths()
        paths = Paths()
        assert paths.global_sessions_dir == custom_sessions

    def test_global_logs_dir_default(self, tmp_path, monkeypatch):
        """Test global logs directory defaults to ~/.atria/logs/."""
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.delenv(ENV_ATRIA_DIR, raising=False)
        monkeypatch.delenv(ENV_ATRIA_LOG_DIR, raising=False)
        reset_paths()
        paths = Paths()
        assert paths.global_logs_dir == tmp_path / ".atria" / "logs"

    def test_global_logs_dir_env_override(self, tmp_path, monkeypatch):
        """Test ATRIA_LOG_DIR environment variable overrides default."""
        custom_logs = tmp_path / "custom-logs"
        monkeypatch.setenv(ENV_ATRIA_LOG_DIR, str(custom_logs))
        reset_paths()
        paths = Paths()
        assert paths.global_logs_dir == custom_logs

    def test_global_cache_dir_default(self, tmp_path, monkeypatch):
        """Test global cache directory defaults to ~/.atria/cache/."""
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.delenv(ENV_ATRIA_DIR, raising=False)
        monkeypatch.delenv(ENV_ATRIA_CACHE_DIR, raising=False)
        reset_paths()
        paths = Paths()
        assert paths.global_cache_dir == tmp_path / ".atria" / "cache"

    def test_global_cache_dir_env_override(self, tmp_path, monkeypatch):
        """Test ATRIA_CACHE_DIR environment variable overrides default."""
        custom_cache = tmp_path / "custom-cache"
        monkeypatch.setenv(ENV_ATRIA_CACHE_DIR, str(custom_cache))
        reset_paths()
        paths = Paths()
    ...
    def test_global_skills_dir(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_skills_dir == tmp_path / ".atria" / "skills"
    ...
    def test_global_agents_dir(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_agents_dir == tmp_path / ".atria" / "agents"
    ...
    def test_global_agents_file(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_agents_file == tmp_path / ".atria" / "agents.json"
    ...
    def test_global_mcp_config(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_mcp_config == tmp_path / ".atria" / "mcp.json"
    ...
    def test_global_repos_dir(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_repos_dir == tmp_path / ".atria" / "repos"
    ...
    def test_global_history_file(self, tmp_path, monkeypatch):
        ...
        reset_paths()
        paths = Paths()
        assert paths.global_history_file == tmp_path / ".atria" / "history.txt"
...
class TestPathsProject:
    ...
    def test_project_dir(self, tmp_path):
        ...
        assert paths.project_dir == tmp_path / ".atria"

    def test_project_settings(self, tmp_path):
        """Test project settings path."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_settings == tmp_path / ".atria" / "settings.json"

    def test_project_skills_dir(self, tmp_path):
        """Test project skills directory."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_skills_dir == tmp_path / ".atria" / "skills"

    def test_project_agents_dir(self, tmp_path):
        """Test project agents directory."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_agents_dir == tmp_path / ".atria" / "agents"

    def test_project_agents_file(self, tmp_path):
        """Test project agents.json file path."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_agents_file == tmp_path / ".atria" / "agents.json"

    def test_project_commands_dir(self, tmp_path):
        """Test project commands directory."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_commands_dir == tmp_path / ".atria" / "commands"

    def test_project_context_file(self, tmp_path):
        """Test project context file (ATRIA.md) at project root."""
        paths = Paths(working_dir=tmp_path)
        assert paths.project_context_file == tmp_path / "ATRIA.md"
...
class TestPathsHelpers:
    ...
    def test_get_agents_dirs_project_priority(self, tmp_path, monkeypatch):
        ...
        global_agents = tmp_path / ".atria" / "agents"
        global_agents.mkdir(parents=True)
        project_agents = project_dir / ".atria" / "agents"
    ...
    def test_session_file(self, tmp_path, monkeypatch):
        ...
        assert session_path == tmp_path / ".atria" / "sessions" / "abc12345.json"
